...
const WatchingShowsList = (props) => {
  ...
  const fetchWatchListShows = async () => {
    const url = "http://localhost:5000/collections/shows/watching";
  ...
  const changeTitlesToShows = async (collectionOfTitles) => {
    ...
    Promise.all(titleToMetadataPromises).then(function (result) {
      ...
      newShowList = result.map((show, index) => {
        show.mongoID = watchListIDs[index];
        return show;
      });
      setWatchListShows(newShowList);
    });
  };
  ...
  const removeFromFavourites = async (idToRemove) => {
    ...
    const url = "http://localhost:5000/collections/shows/watching/update/";
  ...
  useEffect(() => {
    changeIDsToShows(watchListIDs);
  }, [watchListIDs]);

  useEffect(() => {
    changeTitlesToShows(watchListTitles);
  }, [watchListTitles]);
